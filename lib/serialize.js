...
export function Client({ andy, store }) {
	const client = Object.defineProperties(andy, {
		getName: {
			value(jid) {
		...
		sendContact: {
			async value(jid, number, quoted, options = {}) {
				...
				for (let v of number) {
					...
					v = v.replace(/\D+/g, '');
					list.push({
						displayName: andy.getName(v + '@s.whatsapp.net'),
						vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${andy.getName(v + '@s.whatsapp.net')}\nFN:${andy.getName(v + '@s.whatsapp.net')}\nitem1.TEL;waid=${v}:${v}\nEND:VCARD`,
					});
				}
				return andy.sendMessage(
					jid,
					{
						contacts: {
		...
		downloadMediaMessage: {
			async value(message, filename) {
				let media = await downloadMediaMessage(
					...
					{},
					{
						logger: pino({ timestamp: () => `,"time":"${new Date().toJSON()}"`, level: 'fatal' }).child({ class: 'andy' }),
						reuploadRequest: andy.updateMediaMessage,
					}
				);
		...
		cMod: {
			value(jid, copy, text = '', sender = andy.user.id, options = {}) {
				...
				copy.key.fromMe = areJidsSameUser(sender, andy.user.id);
...
export default async function serialize(andy, msg, store) {
	const m = {};
	...
	if (msg.key) {
		...
		m.sender = jidNormalizedUser(m.fromMe ? andy.user.id : m.isGroup ? m.participant : m.from);
	}

	if (m.isGroup) {
		if (!(m.from in store.groupMetadata)) store.groupMetadata[m.from] = await andy.groupMetadata(m.from);
		...
		m.isBotAdmin = m.isGroup && !!m.groupAdmins.find(member => member.id === jidNormalizedUser(andy.user.id));
	}
		...
		if (m.msg?.contextInfo?.quotedMessage) {
			...
			if (m.quoted.message) {
				...
				m.quoted.key = {
					...
					fromMe: areJidsSameUser(jidNormalizedUser(m.msg?.contextInfo?.participant), jidNormalizedUser(andy?.user?.id)),
	...
	m.reply = async (text, options = {}) => {
		if (typeof text === 'string') {
			return await andy.sendMessage(m.from, { text, ...options }, { quoted: m, ephemeralExpiration: m.expiration, ...options });
		} else if (typeof text === 'object' && typeof text !== 'string') {
			return andy.sendMessage(m.from, { ...text, ...options }, { quoted: m, ephemeralExpiration: m.expiration, ...options });
		}
	};
