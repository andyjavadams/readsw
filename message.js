...
/**
 *
 * @param {import('baileys').WASocket} andy
 * @param {any} store
 * @param {import('baileys').WAMessage} m
 */
export default async function message(andy, store, m) {
    try {
        let quoted = m.isQuoted ? m.quoted : m;
        let downloadM = async filename => await andy.downloadMediaMessage(quoted, filename);
        ...
        if (m.message && !m.isBot) {
            console.log(Color.cyan("Dari"), Color.cyan(andy.getName(m.from)), Color.blueBright(m.from));
            console.log(Color.yellowBright("Chat"), Color.yellowBright(m.isGroup ? `Grup (${m.sender} : ${andy.getName(m.sender)})` : "Pribadi"));
            ...
            case "menu":
                {
                    ...
                    Object.entries(menu)
                        .map(([type, command]) => {
                            text += `${dy}${Func.toUpper(type)} Menu${dy}\n\n`;
...
💻 *_Info Server_*
*- Hostname :* ${os.hostname() || andy.user?.name}
...
`.trim();
            ...
            case "q":
                ...
                try {
                    var message = await serialize(andy, await store.loadMessage(m.from, m.quoted.id), store);
            ...
            case "listsw":
                {
                    ...
                    story.forEach(obj => {
                        ...
                        participant = jidNormalizedUser(participant === "status_me" ? andy.user.id : participant);
            ...
            case "upsw":
                if (m.isOwner) {
                    let statusJidList = [
                        jidNormalizedUser(andy.user.id),
            ...
            case "colong":
            case "wmandy":
                if (!m.isQuoted) throw "Balas sticker yang ingin di curi";
            ...
            /*    case "ai":
                {
                    if (!text) throw 'Example: .ai pertanyaan';
                    try {
                        let hasil = await yanzGpt(m.text);
                        m.reply(hasil.choices[0].message.content);
                    } catch (e) {
                        try {
                            let hasil = await bk9Ai(m.text);
                            m.reply(hasil.BK9);
                        } catch (e) {
            ...
            case "link":
                if (!m.isGroup && !m.isBotAdmin) throw "Ga bisa kayaknya aku bukan admin";
                await m.reply("https://chat.whatsapp.com/" + (m.metadata?.inviteCode || (await andy.groupInviteCode(m.from))));
                break;

            case "delete":
            case "del":
                if (quoted.fromMe) {
                    await andy.sendMessage(m.from, { delete: quoted.key });
                } else {
                    if (!m.isBotAdmin) throw "Bot bukan admin";
                    if (!m.isAdmin) throw "Kamu bukan admin bre";
                    await andy.sendMessage(m.from, { delete: quoted.key });
                }
                break;
