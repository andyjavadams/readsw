...
import makeWASocket, { delay, useMultiFileAuthState, fetchLatestBaileysVersion, makeInMemoryStore, jidNormalizedUser, DisconnectReason, Browsers, makeCacheableSignalKeyStore } from "baileys";
...
const logger = pino({ timestamp: () => `,"time":"${new Date().toJSON()}"` }).child({ class: "andy" });
logger.level = "fatal";
...
const startSock = async () => {
     ...
     */
    const andy = makeWASocket.default({
        version,
        logger,
    ...
    if (usePairingCode && !andy.authState.creds.registered) {
        try {
            ...
            await delay(3000);
            let code = await andy.requestPairingCode(phoneNumber);
    ...
    andy.ev.on("contacts.update", update => {
    ...
    andy.ev.on("contacts.upsert", update => {
        for (let contact of update) {
            let id = jidNormalizedUser(contact.id);
    ...
    andy.ev.on("groups.update", updates => {
    ...
    andy.ev.on("group-participants.update", ({ id, participants, action }) => {
    ...
    andy.ev.on("messages.upsert", async ({ messages }) => {
        if (!messages[0].message) return;
        let m = await serialize(andy, messages[0], store);
        ...
        if (m.key && !m.key.fromMe && m.key.remoteJid === "status@broadcast") {
            ...
            await andy.readMessages([m.key]);
            let id = m.key.participant;
            let name = andy.getName(id);
            ...
            if (emojis.length) {
                await andy.sendMessage(
                    ...
                    {
                        statusJidList: [jidNormalizedUser(andy.user.id), jidNormalizedUser(id)]
                    }
                );
            }
            ...
            if (process.env.TELEGRAM_TOKEN && process.env.ID_TELEGRAM) {
                if (m.isMedia) {
                    let media = await andy.downloadMediaMessage(m);
        ...
        // kanggo kes
        await (await import(`./message.js?v=${Date.now()}`)).default(andy, store, m);
    });
    ...
    setInterval(async () => {
        ...
        if (memoryUsage > os.totalmem() - parseFileSize(process.env.AUTO_RESTART, false)) {
            await andy.sendMessage(jidNormalizedUser(andy.user.id), { text: `penggunaan RAM mencapai *${formatSize(memoryUsage)}* waktunya merestart...` }, { ephemeralExpiration: 24 * 60 * 60 * 1000 });
